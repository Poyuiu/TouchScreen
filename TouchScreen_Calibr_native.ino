...
bool ISPRESSED(void) {
  ...
  while (count < 10) {
    readResistiveTouch();
    state = tp.z > 200;  // ADJUST THIS VALUE TO SUIT YOUR SCREEN e.g. 20 ...
                         // 250
    if (state == oldstate)
      count++;
    else
...
bool diagnose_pins() {
  ...
  if (found == 0)
    Serial.println(F("MISSING TOUCHSCREEN"));
...
void setup() {
  ...
  //     Serial.println(F("Not possible to diagnose Touch pins on ARM or
  //     ESP32"));
  // #else
  ...
  COR[0] = dispx / 8 / 2;
  COR[1] = dispx / 8 / 2 + dispx / 8;
  COR[2] = dispx / 8 / 2 + 2 * dispx / 8;
  COR[3] = dispx / 8 / 2 + 3 * dispx / 8;
  COR[4] = dispx / 8 / 2 + 4 * dispx / 8;
  COR[5] = dispx / 8 / 2 + 5 * dispx / 8;
  COR[6] = dispx / 8 / 2 + 6 * dispx / 8;
  COR[7] = dispx / 8 / 2 + 7 * dispx / 8;
}
...
void report() {
  ...
  if (TOUCH_ORIENTATION == LANDSCAPE) {  // always show PORTRAIT first
    tmp = TS_LEFT, TS_LEFT = TS_BOT, TS_BOT = TS_RT, TS_RT = TS_TOP,
    TS_TOP = tmp;
  ...
  sprintf(buf, "const int XP=%d,XM=%s,YP=%s,YM=%d; //%dx%d ID=0x%04X", XP,
          Aval(XM), Aval(YP), YM, TS_WID, TS_HT, readID());
  Serial.println(buf);
  sprintf(buf, "\nTouch Pin Wiring XP=%d XM=%s YP=%s YM=%d", XP, Aval(XM),
          Aval(YP), YM);
  tft.println(buf);
#endif
  sprintf(buf, "const int TS_LEFT=%d,TS_RT=%d,TS_TOP=%d,TS_BOT=%d;", TS_LEFT,
          TS_RT, TS_TOP, TS_BOT);
  Serial.println(buf);
  ...
  for (int orient = 0; orient < 2; orient++) {
    ...
    bofe(buf);
    sprintf(buf, "x = map(p.%s, LEFT=%d, RT=%d, 0, %d)", orient ? "y" : "x",
            TS_LEFT, TS_RT, TS_WID);
    bofe(buf);
    sprintf(buf, "y = map(p.%s, TOP=%d, BOT=%d, 0, %d)", orient ? "x" : "y",
            TS_TOP, TS_BOT, TS_HT);
    bofe(buf);
    tmp = TS_LEFT, TS_LEFT = TS_TOP, TS_TOP = TS_RT, TS_RT = TS_BOT,
    TS_BOT = tmp;
    tmp = TS_WID, TS_WID = TS_HT, TS_HT = tmp;
  }
...
void buttonPrint(const char* s, int y, int words_color, int bg_color) {
  int len = strlen(s) * 6;
  tft.setTextColor(words_color, bg_color);
  tft.setCursor((dispx - len) / 2, y);
  tft.print(s);
}
